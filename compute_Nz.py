...
parser = MyParser(description='Compute N(z) for selected galaxies from SVA1 Gold 1.0 catalogs')
parser.add_argument("cutsfile", help="Textfile file with cuts, check default.cuts for more info")
parser.add_argument("-m","--mask", help="Numpy file with positional index mask, usually created using cutsfile, overrides cutsfile")
parser.add_argument("-i", "--ids", help="Text file (ASCII) with COADD_OBJECTS_ID to be matched (slow), it returns the mask to be used with -m option")
...
if args.mask==None:
    if args.ids==None:
        ...
        print '\n***  Creatin mask from ids from file:  '+args.ids+' ...\n'
else:
